...
private:
	...
	void firstPass(int bitsPerFirstPass, int n, uint *bStart, uint *bSize) {
...

		{
			...
			uint chunk_size = length / num_threads + 1;
			uint *memSize = new uint[num_threads*n];
			parallel_for(int t = 0; t < num_threads; t++) {
				uint* tempSize = memSize + n*t;
				memset(tempSize, 0, n* sizeof(uint));
				uint w = 0;
				uint e = t * chunk_size;
				uint s = std::min((t+1)*chunk_size, length);
				if (s < length) { // Init w correctly 
					for (int i = std::min(length-1, s+4); i >= (int)s; --i) {
						int c = originalInput[i];
						w >>= bitsPerChar;
						w |= shiftedCode[c];	
					}
				}
				for (int i = s - 1; i >= (int)e; --i) {
					int c = originalInput[i];
					w >>= bitsPerChar;
					w |= shiftedCode[c];
					tempSize[w]++;
	...
	int secondPass(uint bitsToSkip, uint nBuckets, uint *bStart, uint *bSize) {
		...
		for (uint i = 0; i < nBuckets; ++i) { // Parallel for
			uint start = bStart[i];
			bucketFlag[start] = 1;
			...
			if (bLen > 1) {
				...
				sorter.sort(bLen, lo, buffer); // Parallel sorting for large bLen
...
//		uint alphaSize =
		...
		int minBitsSortedInSecondPass = inputBasedSort(bitsPerFirstPass); // Takes time
